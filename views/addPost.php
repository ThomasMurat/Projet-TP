...
include 'controllers/addPostController.php'; ?>
    ...
    if(isset($_SESSION['userProfile']) && $_SESSION['userProfile']['role'] == 'administrateur'){ 
        if(isset($message)){ ?>
            <div class="col-10 col-lg-8 text-center jumbotron">
                ...
                <a class="text-center w-100" href="<?= $universeLink ?>&content=postsList">Retour vers la liste des Articles</a>
            </div><?php
        }else {?>
            <form class="offset-1 border border-black col-10" action="<?= $universeLink ?>&content=addPost" method="POST" enctype="multipart/form-data">
                <h2 class="text-center">Ajouter un Article</h2>
                <div id="addPostFormContent" class="row">
                    ...
                    <div class="form-group col-12">
                        ...
                        <input id="username" name="username" type="text" class="form-control" value="<?= $_SESSION['userProfile']['username'] ?>" />
                    ...
                    <div class="form-group col-12">
                        ...
                        <select id="universe" name="universe" class="form-control">
                            ...
                            foreach($universesList as $universe){ ?>
                                <option value="<?= $universe->id ?>"><?= $universe->universe ?></option><?php
                            } ?>
                        </select>
                    ...
                    <div class="form-group text-center col-12">
                        <input type="submit" class="btn btn-primary" name="addPost" value="Ajouter" />
                    </div>
                </div>
            </form><?php
        }
